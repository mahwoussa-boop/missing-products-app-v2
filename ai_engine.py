"""
ai_engine.py — محرك الذكاء الاصطناعي والميزات المتقدمة (الإصدار الشامل V14.0)
═══════════════════════════════════════════════════════════════
...
- توليد وصف احترافي SEO بأسلوب "خبير منتجات مهووس" الدقيق والمطول (1200-1500 كلمة).
...
حاول جلب أكثر من صورة إن أمكن من مواقع موثوقة مثل fragrantica.com أو sephora أو غيرها.

أجب بصيغة JSON فقط:
{{
  "images": [
    {{"url": "رابط الصورة 1", "alt": "{product_name}"}},
    {{"url": "رابط الصورة 2", "alt": "{product_name}"}}
  ],
...
{{
  ...
  "brand": "", "type": "", "year": "", "olfactory_family": ""
}}"""
...
def generate_mahwous_description(product_name, price, fragrantica_data=None):
    """توليد وصف متوافق 100% مع 'خبير وصف منتجات مهووس' SEO (1200-1500 كلمة)."""
    ...
    if fragrantica_data and fragrantica_data.get("success"):
        ...
        frag_info = f"الماركة: {fragrantica_data.get('brand','')} | القمة: {top} | القلب: {mid} | القاعدة: {base} | سنة الإصدار: {fragrantica_data.get('year','')} | الخط العطري: {fragrantica_data.get('olfactory_family','')}"
...
اكتب وصفاً حصرياً، راقياً، ومطولاً (بين 1200 إلى 1500 كلمة) لهذا المنتج:
المنتج: {product_name}
السعر: {price} ريال
{frag_info}

يجب أن تلتزم بالهيكلة الإلزامية التالية (استخدم Markdown):

## [H1] [اسم المنتج الكامل]: [عنوان تسويقي جذاب يتضمن الكلمة الرئيسية]
[مقدمة عاطفية تسويقية مطولة (150-200 كلمة) تبرز جاذبية العطر ولماذا يجب اقتناؤه، مع تكرار الكلمة الرئيسية بشكل طبيعي.]

### تفاصيل المنتج
* **الماركة:** [اسم الماركة]
* **النوع:** عطر.
* **الجنس:** [نسائي/رجالي/للجنسين]
* **الحجم:** [حجم العطر]
* **التركيز:** [مثال: أو دو بارفان]
* **سنة الإصدار:** [سنة الإصدار]
* **الخط العطري:** [العائلة العطرية]

### رحلة العطر: النفحات والمكونات
[وصف حسي وتفصيلي للمكونات وكيف تتدرج، لا يقل عن 300 كلمة.]
* **مقدمة العطر:** [مكونات القمة مع وصف تأثيرها]
* **قلب العطر:** [مكونات القلب مع وصف تناغمها]
* **قاعدة العطر:** [مكونات القاعدة مع وصف ثباتها وعمقها]

### لمسة خبير من مهووس
[فقرة متخصصة تصف شخصية العطر، لمن يناسب، وفي أي وقت يفضل ارتداؤه (200 كلمة).]

### لماذا تتسوق من متجر مهووس؟
* **ضمان أصالة 100%:** جميع عطورنا أصلية ومن مصادرها الرسمية.
* **تغليف فاخر:** نهتم بأدق التفاصيل لتقديم العطر كتحفة فنية.
* **توصيل سريع:** شحن آمن وسريع لجميع مناطق المملكة العربية السعودية.
* **تجربة تسوق موثوقة:** آلاف العملاء يثقون بمهووس كوجهتهم العطرية الأولى.

### الأسئلة الشائعة (FAQ)
[اكتب 6-8 أسئلة وأجوبة تفصيلية ومقنعة حول العطر.]
1. **س: هل العطر مناسب للاستخدام اليومي أم للمناسبات؟**
   ج: [إجابة مقنعة ومفصلة]
2. **س: ما هو مدى ثبات وفوحان العطر؟**
   ج: [إجابة مقنعة ومفصلة تعتمد على التركيز والمكونات]
[... أكمل باقي الأسئلة ...]

### عالمك العطري يبدأ من مهووس
[خاتمة قوية (100 كلمة) تتضمن دعوة للشراء (CTA) وتعزيز الثقة بمتجر مهووس.]

**قواعد إضافية:**
- لا تستخدم الإيموجي نهائياً.
- استخدم Bold للكلمات المفتاحية المهمة.
- اللغة: عربية فصحى راقية ممزوجة بأسلوب تسويقي ودود.
- الطول الإجمالي: يجب أن يقترب من 1500 كلمة.
"""

    try:
        response = model.generate_content(prompt)
        text = re.sub(r'```html|```markdown|```', '', response.text).strip()
        if len(text) < 500: # حماية في حال كان الرد قصيراً جداً
             # محاولة ثانية بطلب تأكيد الطول
             response = model.generate_content(prompt + "\nتنبيه: يجب أن يكون النص طويلاً جداً ومفصلاً ليتجاوز 1200 كلمة.")
             text = re.sub(r'```html|```markdown|```', '', response.text).strip()
        return text
    except Exception as e:
        return f"## {product_name}\n\nعطر رائع متوفر الآن في متجر مهووس بسعر {price} ريال. احصل عليه اليوم وعش تجربة فريدة مع مهووس."
